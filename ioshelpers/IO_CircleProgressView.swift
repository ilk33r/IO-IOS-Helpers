...
@IBDesignable
class IO_CircleProgressView: UIView {
	...
	*/
	...
	private func calculatePathBounds(rect: CGRect) {
		...
		if(rect.size.width > rect.size.height) {
			...
			progressSize = CGSize(width: rect.size.height - (self.lineWidth * 4), height: rect.size.height - (self.lineWidth * 4))
			
		}else if(rect.size.width > rect.size.height) {
			progressSize = CGSize(width: rect.size.width - (self.lineWidth * 4), height: rect.size.width - (self.lineWidth * 4))
		}else{
			
			progressSize = CGSize(width: rect.size.width - (self.lineWidth * 4), height: rect.size.width - (self.lineWidth * 4))
		}
		
		let progressPos = CGPoint(x: (self.lineWidth * 1.5), y: (self.lineWidth * 1.5))
